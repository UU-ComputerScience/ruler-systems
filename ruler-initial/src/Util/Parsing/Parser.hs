...
module Util.Parsing.Parser where
...
instance (Eq a, Show a) => Provides  (Str  a)  a  a where
       splitState a k (Str  tts   msgs pos  ok) 
          = let ins exp = Just  (5, k a            (Str tts         (msgs ++ [Inserted a           pos  exp]) pos     False))
                del exp = Just  (5, splitState a k (Str (tail tts)  (msgs ++ [Deleted  (head tts)  pos  exp]) (pos+1) True ))
            in case tts of
               (t:ts)  ->  if a == t 
                           then  Step 1 (k t (Str ts msgs (pos + 1) True))
                           else  Fail [show a] (ins: if ok then [del] else [])
               []      ->  Fail [show a] [ins]

instance (Show a) => Provides  (Str  a)  (a -> Bool, String, a)  a where
       splitState (p, msg, a) k (Str  tts   msgs pos  ok) 
          = let ins exp = Just  (5, k a                (Str tts         (msgs ++ [Inserted a           pos  exp]) pos     False))
                del exp = Just  (5, splitState (p,msg, a) k (Str (tail tts)  (msgs ++ [Deleted  (head tts)  pos  exp]) (pos+1) True ))
            in case tts of
               (t:ts)  ->  if p t 
                           then  Step 1 (k t (Str ts msgs (pos + 1) True))
...
instance (Ord a, Show a) => Provides  (Str  a)  (a,a)  a where
       splitState a@(low, high) k (Str  tts   msgs pos  ok) 
          = let ins exp = Just  (5, k low            (Str tts         (msgs ++ [Inserted low         pos  exp]) pos     False))
                del exp = Just  (5, splitState a k   (Str (tail tts)  (msgs ++ [Deleted  (head tts)  pos  exp]) (pos+1) True ))
            in case tts of
               (t:ts)  ->  if low <= t && t <= high 
                           then  Step 1 (k t (Str ts msgs (pos + 1) True))
                           else  Fail [show low ++".."++show high] (ins: if ok then [del] else [])
               []      ->  Fail [show low ++".."++show high] [ins]
