-- Edited version 0.4 from hackage.
-- Replaced the showAttr function to generate
-- a html-label
-- changed the showDot function.
-- bugfix edge'
...
showDot :: String -> Dot a -> (String, a)
showDot header (Dot dm) = case dm 0 of
...
showAttr (name,val) = name ++ "=" ++ valEsc
  where valEsc | valIsHtml = val
               | otherwise = "\""   ++ foldr showsDotChar "" val ++ "\""
        valIsHtml = name == "label" && not (null val) && head val == '<'
